...
import { getClientById } from "@/app/service/superAdminAPI";
...
import { getClientPerformanceSuperAdmin } from "@/app/service/superAdminAPI";
...
export default function GetClientDetail({
  ...
  const fetchClientStats = async () => {
    try {
      ...
      const data = await getClientPerformanceSuperAdmin(clientId);
      setClientStats(data);
    } catch (error) {
                  ...
                  >
                    ...
                    {client.outstandingBalance !== undefined && (
                      <div>
                        ...
                        >
                          ${Number(client.outstandingBalance).toFixed(2)}
                        </Text>
                      </div>
                    )}
                  </Space>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:justify-center">
                    {hasPermission("read_client") && (
                      <Button
                        size="large"
                        icon={<TeamOutlined />}
                        onClick={() => router.push("/super-admin/get-clients")}
                    ...
                    {hasPermission("read_case") && (
                      <Button
                        ...
                        onClick={() => router.push(`/super-admin/get-cases`)}
